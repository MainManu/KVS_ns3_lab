...
public:
                                ...
                                uint16_t staId)
  {
    ...
    static Time last_time = Seconds(0); // Time of the last packet (initially 0)
    static Time aggregated_start_time = Time(0);
    static uint32_t aggregated_size_b = 0;
    static uint32_t aggregated_count = 0;
    Time now = Simulator::Now();     // Current time
    Time interval = now - last_time; // Time interval between packets

    // handle aggregation
    switch (aMpdu.type) {
      case NORMAL_MPDU:
        // handle non-aggregated packets
        if (is_first_packet) {
          // handle first packet to avoid divide by zero
          map_time_to_rxDr_bps[now] = 0;
          is_first_packet = false;
        } else {
          // just a regular packet
          map_time_to_rxDr_bps[now] =
      ...
      case FIRST_MPDU_IN_AGGREGATE:
        // handle first mpdu to avoid divide by zero
        aggregated_start_time = now;
        aggregated_size_b = packet->GetSize() * 8;
        aggregated_count = 1;
        break;
      case MIDDLE_MPDU_IN_AGGREGATE:
        // still in the same mpdu
        aggregated_size_b += packet->GetSize() * 8;
        aggregated_count++;
        break;
      case LAST_MPDU_IN_AGGREGATE:
        aggregated_size_b += packet->GetSize() * 8;
        //  next mpdu found, time to calculate the data rate
        map_time_to_rxDr_bps[aggregated_start_time] =
          (aggregated_size_b-(IP4_OVERHEAD+UDP_OVERHEAD)*aggregated_count) / (aggregated_start_time - last_time).GetSeconds();
        last_time = now;
        break;
      default:
        break;
                    ...
                    enum WifiPreamble preamble)
  {
    ...
    if (interval.GetSeconds() ==
        0) // handle first packet to avoid divide by zero
    {
      map_time_to_rxDr_bps[now] = 0;
      return;
    }
    ...
    bool isFirstInMpdu =
      aggregated_start_time.IsZero() && macHeader.IsQosData();
    bool isParrtOfMpdu =
      sequence_number == last_sequnce_number && macHeader.IsQosData();
    ...
    if (isFirstInMpdu || isParrtOfMpdu) {
      ...
      double dataRate =
        ((packet->GetSize() * 8) - IP4_OVERHEAD - UDP_OVERHEAD) /
        interval.GetSeconds();
  ...
  std::map<Time, double> convert_map_to_Mps()
  {
    ...
    for (const auto& pair : map_time_to_rxDr_bps) {
      // if (count < 20)
      //     std::cout << "convert_map_to_Mps: " << pair.first.GetSeconds() <<
      //     "seconds "
      //               << pair.second << "result:" << pair.second / 1000 / 1000
      //               << std::endl;
...
main(int argc, char* argv[])
{
  ...
  cmd.AddValue("csv_export_path",
               ...
               csv_export_path);
  cmd.AddValue(
    "propagationModel", "propagation model to use", propagationModel);
  cmd.AddValue(
    "export_summary", "export summary to csv (true,false)", export_summary);
  cmd.AddValue(
    "export_rx_pwr", "export rx power to csv (true,false)", export_rx_pwr);
  cmd.AddValue(
    "export_rx_dr", "export rx data rate to csv (true,false)", export_rx_dr);

  cmd.Parse(argc, argv);

  // say hello so I know when compilation is done
  std::cout << std::setw(80)
            << "===starting wifi-experiment.cc===" << std::endl;
  ...
  wifi.SetRemoteStationManager(
    "ns3::MinstrelHtWifiManager"); // Ht needed for 802.11n
  ...
  std::cout << "Average Rx power: " << rx_pwr.get_avg_rx_pwr_dbm() << " dBm"
            << std::endl;
  ...
  if (csv_export_path != "") {
    if (export_summary) {
      ...
      for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i =
        ...
        csvFile << i->first << "," << t.sourceAddress << ","
                << t.destinationAddress << "," << std::fixed
                << std::setprecision(2) << receivedDataRate << ","
